...
script.Parent.MouseButton1Down:Connect(function()
	...
	if workspace.plam[plrn].map.Value ~= workspace.plam[game.Players.LocalPlayer.Name].map.Value then
		if game.ReplicatedFirst.maps:FindFirstChild(workspace.plam[plrn].map.Value) then
		...
		elseif workspace.plam[plrn].map.Value == "not MAKE" then
			local origtext = script.Parent.Parent.TextLabel.Text
			script.Parent.Parent.TextLabel.Text = "they in 'not MAKE' it wont work lmao"
			spawn(function()
				wait(5)
				script.Parent.Parent.TextLabel.Text = origtext
			end)
		else
			local origtext = script.Parent.Parent.TextLabel.Text
			script.Parent.Parent.TextLabel.Text = "their map not valid?? lmao"
			spawn(function()
				wait(5)
				script.Parent.Parent.TextLabel.Text = origtext
			end)
